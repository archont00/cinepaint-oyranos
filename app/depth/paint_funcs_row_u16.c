...
static void copy_row_rgb_to_float16_rgb(PixelRow* src_row, PixelRow * dest_row) {
   ...
   if (tag_alpha (stag) == ALPHA_YES) {
      if (tag_alpha (dtag) == ALPHA_YES) {
         while (w--) {
            d[0] = FLT16 (s[0] / (gfloat) 65535, u);
            d[1] = FLT16 (s[1] / (gfloat) 65535, u);
            d[2] = FLT16 (s[2] / (gfloat) 65535, u);
            d[3] = FLT16 (s[3] / (gfloat) 65535, u);
            s += 4;
            d += 4;
            }
         }
...
static void copy_row_rgb_to_float16_gray (PixelRow* src_row, PixelRow* dest_row) {
   ...
   if (tag_alpha (stag) == ALPHA_YES) {
      if (tag_alpha (dtag) == ALPHA_YES) {
         while (w--) {
            d[0] = FLT16 (INTENSITY ((s[0] / (gfloat) 65535), (s[1] / (gfloat) 65535), (s[2] / (gfloat) 65535)), u);
            d[1] = FLT16 (s[3] / (gfloat) 65535, u);
            s += 4;
            d += 2;
            }
         }
  ...
  if (tag_alpha (stag) == ALPHA_YES)
    {
      if (tag_alpha (dtag) == ALPHA_YES)
        while (w--)
          {
            d[0] = INTENSITY (s[0] >> 1, s[1] >> 1, s[2] >> 1);
            d[1] = s[3] >> 1;
            s += 4;
            d += 2;
          }
...
static void copy_row_gray_to_float16_rgb (PixelRow* src_row, PixelRow* dest_row) {
   ...
   if (tag_alpha (stag) == ALPHA_YES) {
      if (tag_alpha (dtag) == ALPHA_YES) {
         while (w--) {
            d[0] = d[1] = d[2] = FLT16 (s[0] / (gfloat) 65535, u);
              ...
              )
{
  ...
  switch (tag_precision (pixelrow_tag (dest_row))) {
    ...
    case PRECISION_NONE:
    default:
        g_warning ("unsupported dest precision in copy_row_u16() of [%d]", tag_precision (pixelrow_tag (dest_row)));
      return;
    }
