...
static void *      shear_tool_shear   (GImage *, CanvasDrawable *, Canvas *, int, Matrix);
  ...
  switch (state)
      ...
      if (!transform_info)
	{
	  transform_info = info_dialog_new (_("Shear Information"),
					    ...
					    _("UNKNOWN"));
	  info_dialog_add_field (transform_info, _("X Shear Magnitude: "), xshear_buf);
	  info_dialog_add_field (transform_info, _("Y Shear Magnitude: "), yshear_buf);
	}
      direction_unknown = 1;
