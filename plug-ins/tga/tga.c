 ...
 *
 * $Id: tga.c,v 1.2 2006/10/04 18:24:58 beku Exp $
...
#ifdef PROFILE
  times (&tbuf2);
  d_printf ("TGA: %s profile: %ld user %ld system\n", name,
...
load_image (char *filename)
{
  ...
  if (!fp) {
      d_printf ("TGA: can't open \"%s\"\n", filename);
      return -1;
    }

  name_buf = g_malloc (strlen (filename) + 11);
  sprintf (name_buf, "%s %s:", _("Loading"), filename);
  ...
  if (fseek (fp, 0L - (sizeof (tga_footer)), SEEK_END)
    ...
    {
      d_printf ("TGA: Cannot read footer from \"%s\"\n", filename);
      return -1;
    }
  ...
  if (memcmp (tga_footer.signature, TGA_SIGNATURE,
    ...
    {
      if (verbose)
	d_printf ("TGA: found New TGA\n");
    }
  else if (verbose)
    d_printf ("TGA: found Original TGA\n");
#endif

  if (fseek (fp, 0, SEEK_SET) ||
      fread (&hdr, sizeof (hdr), 1, fp) != 1) {
    d_printf("TGA: Cannot read header from \"%s\"\n", filename);
    return -1;
  }
  ...
  if (hdr.idLength && fseek (fp, hdr.idLength, SEEK_CUR))
    {
      d_printf ("TGA: Cannot skip ID field in \"%s\"\n", filename);
      return -1;
    }
  ...
  if (verbose > 1)
    {
      ...
      d_printf ("TGA: std_fread %d (total %d)\n",
...
std_fwrite (guchar *buf, int datasize, int nelems, FILE *fp)
{
  ...
  if (verbose > 1)
    {
      totbytes += nelems * datasize;
      d_printf ("TGA: std_fwrite %d (total %d)\n",
	      nelems * datasize, totbytes);
    }
#endif
...
rle_fread (guchar *buf, int datasize, int nelems, FILE *fp)
{
  ...
  while (j < buflen)
...
#ifdef VERBOSE
	  if (verbose)
	    d_printf ("TGA: hit EOF while looking for count\n");
#endif
	  return j / datasize;
	}
	...
	d_printf ("TGA: needed to use statebuf for %d bytes\n", buflen - j);
#endif
...
#ifdef VERBOSE
	      if (verbose)
		d_printf ("TGA: EOF while reading %d/%d element RLE packet\n",
			bytes, datasize);
#endif
	      return j / datasize;
	    }
...
#ifdef VERBOSE
	      if (verbose)
		d_printf ("TGA: EOF while reading %d/%d element raw packet\n",
			bytes, datasize);
#endif
	      return j / datasize;
	    }
  ...
  if (verbose > 1)
    {
      d_printf ("TGA: rle_fread %d/%d (total %d)\n",
...
rle_fwrite (guchar *buf, int datasize, int nelems, FILE *fp)
{
  ...
  if (begin < buf + buflen)
      ...
      if (verbose > 1)
	{
	  totbytes += datasize;
	  if (verbose > 2)
	    d_printf ("TGA: FINAL raw packet %d/%d\n", datasize, totbytes);
	}
#endif
      if (fputc (0, fp) == EOF ||
	  fwrite (begin, datasize, 1, fp) != 1)
  ...
  if (verbose > 1)
    {
      d_printf ("TGA: rle_fwrite %d/%d (total %d)\n",
...
ReadImage (FILE *fp, struct tga_header *hdr, char *filename)
{
  ...
  if (abpp + pbpp > bpp)
    {
      d_printf ("TGA: %d bit image, %d bit alpha is greater than %d total bits per pixel\n",
	      pbpp, abpp, bpp);

      /* Assume that alpha bits were set incorrectly. */
      abpp = bpp - pbpp;
      d_printf ("TGA: reducing to %d bit alpha\n", abpp);
    }
  else if (abpp + pbpp < bpp)
    {
      d_printf ("TGA: %d bit image, %d bit alpha is less than %d total bits per pixel\n",
	      pbpp, abpp, bpp);

      /* Again, assume that alpha bits were set incorrectly. */
      abpp = bpp - pbpp;
      d_printf ("TGA: increasing to %d bit alpha\n", abpp);
    }
  ...
  switch (hdr->imageType)
...
#ifdef VERBOSE
      if (verbose)
	d_printf ("TGA: %d bit indexed image, %d bit alpha (%d bit indices)\n",
		pbpp, abpp, bpp);
#endif
      ...
      if (bpp != 8)
	{
	  ...
	  d_printf ("TGA: index sizes other than 8 bits are unimplemented\n");
	  return -1;
	}
...
#ifdef VERBOSE
      if (verbose)
	d_printf ("TGA: %d bit grayscale image, %d bit alpha\n",
		pbpp, abpp);
#endif
...
#ifdef VERBOSE
      if (verbose)
	d_printf ("TGA: %d bit color image, %d bit alpha\n", pbpp, abpp);
#endif
    ...
    default:
      d_printf ("TGA: unrecognized image type %d\n", hdr->imageType);
      return -1;
    }
  ...
  if ((abpp && abpp != 8) ||
      ...
      (itype == GRAY && pbpp != 8))
    {
      ...
      d_printf ("TGA: channel sizes other than 8 bits are unimplemented\n");
      return -1;
    }
  ...
  if (itype == INDEXED)
	...
	{
	  d_printf ("TGA: indexed image has invalid color map type %d\n",
		  hdr->colorMapType);
	  return -1;
	}
    }
    ...
    {
      d_printf ("TGA: non-indexed image has invalid color map type %d\n",
	      hdr->colorMapType);
      return -1;
    }
  ...
  if (hdr->colorMapType == 1)
...
#ifdef VERBOSE
      if (verbose)
	d_printf ("TGA: reading color map (%d + %d) * (%d bits)\n",
		index, length, hdr->colorMapSize);
#endif
      if (length == 0)
	{
	  d_printf ("TGA: invalid color map length %d\n", length);
	...
	{
	  d_printf ("TGA: error reading colormap (ftell == %ld)\n", ftell (fp));
	  gimp_image_delete (image_ID);
	  return -1;
	}
...
#ifdef VERBOSE
      if (verbose > 1)
	d_printf ("TGA: reading %dx(%d+%d)x%d pixel region\n", width,
	  ...
	  if (!badread)
	    {
	      ...
	      d_printf ("TGA: error reading (ftell == %ld)\n", ftell (fp));
	      badread = 1;
	    }

#if 0
  verbose = 2;
  d_printf ("TGA: debug %d\n", getpid ());
  kill (getpid (), 19);
#endif
  ...
  if (fgetc (fp) != EOF)
    d_printf ("TGA: too much input data, ignoring extra...\n");
	    ...
	    gint32  drawable_ID)
{
  ...
  sprintf(name_buf, "%s %s:", _("Saving"), filename);
  gimp_progress_init(name_buf);
  g_free(name_buf);
  ...
  if (hdr.imageType == TGA_TYPE_MAPPED_RLE ||
      hdr.imageType == TGA_TYPE_MAPPED)
    {
      ...
      if (colors > 256)
	{
	  d_printf ("TGA: cannot handle colormap with more than 256 colors (got %d)\n", colors);
	  return FALSE;
	}
...
#ifdef VERBOSE
      if (verbose > 1)
	d_printf ("TGA: writing %dx(%d+%d)x%d pixel region\n",
...
save_dialog ()
{
  ...
  dlg = gtk_dialog_new ();
  gtk_window_set_title (GTK_WINDOW (dlg), _("Save as Tga"));
