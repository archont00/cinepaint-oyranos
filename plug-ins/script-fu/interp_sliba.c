...
init_sliba_version (void)
{
  setvar (cintern ("*sliba-version*"),
	  cintern ("$Id: interp_sliba.c,v 1.2 2006/12/31 09:02:35 beku Exp $"),
	  NIL);
}
...
array_prin1 (LISP ptr, struct gen_printio *f)
{
  ...
  switch (ptr->type)
	...
	{
	  g_ascii_dtostr(tkbuffer, 12, ptr->storage_as.double_array.data[j]);
	  gput_st (f, tkbuffer);
