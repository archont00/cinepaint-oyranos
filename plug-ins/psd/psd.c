/* PSD Plugin for CinePaint/FilmGimp
 *  Original source from GIMP 1.3.11 source tree, modified to compile with
 *  `filmgimptool --build` by Tom Huffman <liquidphoto@ameritech.net>
 *  in March 2003
 */
 ...
 * Revision history:
 *
 *  2005.12.15 / v2.0.7 / Kai-Uwe Behrmann
 *       allow native cmyk for CinePaint
 *
...
#define IFDBG if (PSD_DEBUG) printf("%s:%d %s() ",__FILE__,__LINE__,__func__); if (PSD_DEBUG) 
...
void extract_data_and_channels(guchar* src, gint gimpstep, gint psstep,
			       ...
			       gint width, gint height, int cmyk)
{
  guchar* primary_data;
  guchar* aux_data;
  ...
  primary_data = xmalloc(width * height * gimpstep);
  {
    ...
    if(cmyk) {
      ...
      for (i = 0; i < height; i++) {
        for (j = 0; j < width; j++) {
            int pos = (i*width+j)*gimpstep;
            primary_data[pos+0] = 255-primary_data[pos+0];
            primary_data[pos+1] = 255-primary_data[pos+1];
            primary_data[pos+2] = 255-primary_data[pos+2];
            primary_data[pos+3] = 255-primary_data[pos+3];
        }
      }
    }
		      ...
		      gint width, gint height)
{
  ...
  aux_data = xmalloc(width * height);
  {
	...
	channel_ID = gimp_channel_new(image_ID,
				      psd_image.aux_channel[chan-(psstep-num_wanted)].name ? psd_image.aux_channel[chan-(psstep-num_wanted)].name : _("Unnamed channel"),
				      width, height,
				      100.0, colour);
	gimp_image_add_channel(image_ID, channel_ID, 0);
	gimp_channel_set_visible(channel_ID, FALSE);
  ...
  if (want_aux || (psd_image.num_layers==0)) /* Photoshop2-style: NO LAYERS. */
    {
      ...
      IFDBG printf("Image data %ld chars, channels %d\n", PSDheader.imgdatalen, 
                    PSDheader.channels);
      ...
      if ((PSDheader.bpp != 8) && (PSDheader.bpp != 1))
	{
      ...
      IFDBG printf(
	     "psd:%d gimp:%d gimpbase:%d cmyk:%d\n",
	     imagetype,
	  ...
	  if (channels == step) /* gimp bpp == psd bpp */
	      ...
	      if (psd_type_to_gimp_type(imagetype)==INDEXEDA_IMAGE)
		{
          ...
          for (j = 0; j < PSDheader.columns; j++) {
            int pos = (i*PSDheader.columns+j)*step;
            dest[pos+0] = 255-dest[pos+0];
            dest[pos+1] = 255-dest[pos+1];
            dest[pos+2] = 255-dest[pos+2];
            dest[pos+3] = 255-dest[pos+3];
          }
          }
          }
          printf("step: %d cmyk: %d\n", step, cmyk);
	      gimp_pixel_rgn_init (&pixel_rgn, drawable,
				   0, 0, drawable->width, drawable->height,
				   TRUE, FALSE);
	  ...
	  else
	    {
	      ...
	      extract_data_and_channels(dest, channels, step,
					image_ID, drawable,
					drawable->width, drawable->height, cmyk);	  
	    }
	}
...
/* TWH- March 2003 - code to add guides, not in libfilmgimp v0.16
