...
/*------------------------------------------------------------------------*/
/* added from Marti's jpegicc.c by ku.b */
...
int read_icc_profile2(j_decompress_ptr cinfo, JOCTET **icc_data_ptr,
		  unsigned int *icc_data_len)
{
...
# if HAVE_OY
  {
    ...
    case JCS_YCbCr:
         if(lIsITUFax)
           profile_name = malloc(24); sprintf(profile_name,"ITULab.icc");
