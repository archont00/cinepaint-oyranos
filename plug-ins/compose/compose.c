...
static gint32    create_new_image (char *filename, guint width, guint height,
                   GDrawableType gdtype, GImageType itype, gint32 *layer_ID, TileDrawable **drawable,
                   GPixelRgn *pixel_rgn);
...
{int width, height, tile_height, scan_lines;
 ...
 TileDrawable *drawable_src[MAX_COMPOSE_IMAGES], *drawable_dst;
 GPixelRgn pixel_rgn_src[MAX_COMPOSE_IMAGES], pixel_rgn_dst;
  ...
  for (j = 1; j < num_images; j++)
  {
    ...
    {
      e_printf ("compose: images have different size\n");
      return -1;
    }
  }
  ...
  for (j = 0; j < num_images; j++)
  {
    ...
    {
      e_printf ("compose: error in getting layer IDs\n");
      return (-1);
    }
    ...
    {
      e_printf ("compose: image is not a gray image\n");
      return (-1);
    }
    ...
    if (src[j] == NULL)
    {
      e_printf ("compose: not enough memory\n");
      return (-1);
    }
  }
  ...
  /* Create new image */
  {
    GDrawableType drawable_type = gimp_drawable_type (g32[0]);
    GImageType image_type = gimp_image_base_type (g32[0]);
    int alpha = gimp_drawable_has_alpha(g32[0]);
