/*
 * HdrCalctor_RGB_U16_as_U8.hpp  --  Part of the CinePaint plug-in "Bracketing_to_HDR"
 *
...
*/
#ifndef HdrCalctor_RGB_U16_as_U8_hpp
#define HdrCalctor_RGB_U16_as_U8_hpp
...
/**===========================================================================
  ...
  @class  HdrCalctor_RGB_U16_as_U8
  
  Hdr-Calctor for \c PackImgScheme2D objects of type RGB_U16. HDR-merging in U16,
   but computation of response curves and the for it needed follow-up values in U8.
  ...
  - ACHTUNG: Calctor hat fuer Fortleben des Packs selbst zu sorgen, also Kopie
   des im Ctor mitgeteilten Pack's ziehen, nicht nur Referenz nehmen!
   \n
...
*============================================================================*/
class HdrCalctor_RGB_U16_as_U8 : public HdrCalctorBase
{
public:
    ...
    */    
    typedef PackImgScheme2D_RGB_U16   Pack;
...
private:
    ...
    FollowUpValues_RGB_U16_as_U8  followUp_;

    //  Pointer to our *U8* weight function for response computation
    WeightFunc_U8*  pWeightResp_;
    
    //  Pointer to our *U16* weight function for *merging*
    const WeightFunc_U16*  pWeightMerge_;

public:
     
    /*  Ctor... (`const Pack &' - we take a copy) */
    HdrCalctor_RGB_U16_as_U8 (  const Pack &,
                                WeightFunc_U8 &         response,         
                                const WeightFunc_U16 &  merge );

    /*  Dtor */
    ~HdrCalctor_RGB_U16_as_U8()                         {DTOR("");}
    ...
    const WeightFunc_U16 & weightMerge()  const         {return *pWeightMerge_;}
    
    /*  Set our weight function pointers */
    void setWeightPtrResp (WeightFunc_U8 *p)            {assert(p); pWeightResp_ = p;}
    void setWeightPtrMerge (const WeightFunc_U16 *p)    {assert(p); pWeightMerge_ = p;}
...
#endif  // HdrCalctor_RGB_U16_as_U8_hpp
// END OF FILE
