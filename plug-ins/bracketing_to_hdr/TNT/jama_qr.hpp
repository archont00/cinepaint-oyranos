/**
  From jama_qr.h with micro changes by Hartmut Sbosny (HS), see below.
*/
#ifndef JAMA_QR_H
#define JAMA_QR_H
...
#include "tnt_math_utils.hpp"   // hypot<>), [abs<>() - unused]
...
<p>
    Changes by H.S:
     - isFullRank(): test not to 0 but to a small epsilon.
     - All member functions "outlined" instead of inline.
     - More readable code styling in a strict "tab-4" manner.
*/
...
*/
template <class Real>
QR<Real>::QR(const TNT::Array2D<Real> &A)
{
    QR_ = A.copy();
    m = A.dim1();
    n = A.dim2();
...
/**
    ...
    HS: test to eps instead to 0.
    The choice of `eps' (or `tol') not so critcal as in SVD, because only 
     anyone value needs to be .<. eps to decide: isFullRank=false.
*/
template <class Real>
int QR<Real>::isFullRank() const
{
    double eps = 1.E-13;
    //std::cout << "isFullRank(): Rdiag = " << Rdiag;
...
*/
template <class Real>
TNT::Array2D<Real> QR<Real>::getHouseholder (void)  const
{
...
*/
template <class Real>
TNT::Array2D<Real> QR<Real>::getR() const
{
...
*/
template <class Real>
TNT::Array2D<Real> QR<Real>::getQ() const
{
    int i=0, j=0, k=0;
...
*/
template <class Real>
TNT::Array1D<Real> QR<Real>::solve(const TNT::Array1D<Real> &b) const
{
...
*/
template <class Real>
TNT::Array2D<Real> QR<Real>::solve (const TNT::Array2D<Real> &B) const
{
    if (B.dim1() != m)          /* arrays must be conformant */
